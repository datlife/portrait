...
from tensorflow.python.keras.layers import DepthwiseConv2D, Reshape
from tensorflow.python.keras.layers import Activation, AvgPool2D, Add
...
def MobilenetV2(inputs, alpha=1.0, num_classes=1000, include_top=True):
    ...
    with tf.variable_scope('MobilenetV2'):
        with tf.variable_scope('Conv'):
            ...
            x = Conv2D(first_block, 3, strides=2, padding='same', use_bias=False, name='Conv2D')(inputs)
            x = BatchNormalization(epsilon=1e-3, momentum=0.999, name='BatchNorm')(x)
        ...
        if include_top:
            with tf.variable_scope('Predictions'):
                x = AvgPool2D((7, 7))(x)
                x = Conv2D(num_classes, 1, activation='softmax', use_bias=True)(x)
